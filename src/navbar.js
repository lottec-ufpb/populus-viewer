...
var DocumentNavbar = /** @class */ (function (_super) {
    ...
    function DocumentNavbar(props) {
        ...
        _this.prevPage = function (_) {
            ...
            if (_this.props.pageFocused > 1) {
                history_js_1.default.push("/".concat(encodeURIComponent(_this.props.resourceAlias), "/").concat(Math.max(1, _this.props.pageFocused - (1 + sparePages)), "/"));
                _this.props.contentContainer.current.scrollTop = _this.props.contentContainer.current.scrollHeight;
            }
        };
        _this.nextPage = function (_) {
            var sparePages = _this.props.content.current.state.showSecondary ? 1 : 0;
            if (_this.props.pageFocused + sparePages < _this.props.total) {
                history_js_1.default.push("/".concat(encodeURIComponent(_this.props.resourceAlias), "/").concat(_this.props.pageFocused + sparePages + 1, "/"));
                _this.props.contentContainer.current.scrollTop = 0;
            }
        };
        ...
        _this.setSearchFocus = function (searchFocused) {
            if (searchFocused) {
                _this.props.setNavHeight(150);
                _this.setState({ searchFocused: searchFocused, moreOptionsVisible: true });
            }
            else
                _this.setState({ searchFocused: searchFocused });
        };
    ...
    DocumentNavbar.prototype.render = function (props, state) {
        ...
        if (props.contentWidthPx) { // don't render until width is set
          ...
          <Pages total={props.total} handleClick={this.handleClick} roomId={props.roomId} currentPageElement={this.currentPageElement} visibility={state.pageViewVisible} typing={state.typing} current={props.pageFocused}/>
        ...
        <div class="nav-button-wrapper top-wrapper">
          ...
          <tooltip_js_1.default content="Add annotation (Alt + a)" offset={this.toolTipOffset}>
            <button disabled={(props.hasSelection || ((_a = props.pindropMode) === null || _a === void 0 ? void 0 : _a.x)) ? null : "disabled"} onclick={props.openAnnotation}>{Icons.addAnnotation}
            </button>
          </tooltip_js_1.default>
          ...
          <form onSubmit={this.handleSubmit}>
            <tooltip_js_1.default content="Show page navigation" offset={this.toolTipOffset}>
              <button class={state.pageViewVisible ? "nav-toggled" : null} onclick={this.togglePageNav}>{Icons.page}
              </button>
            </tooltip_js_1.default>
            <input type="text" ref={this.pageInput} value={state.pageFocused ? state.value : props.pageFocused} onblur={this.handlePageBlur} onfocus={this.handlePageFocus} oninput={this.handleInput}/>
            <span>/</span>
        ...
        <button onClick={(_b = props.content.current) === null || _b === void 0 ? void 0 : _b.toggleSecondary}>{Icons.columns}</button>
          </tooltip_js_1.default>
